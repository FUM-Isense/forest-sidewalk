...
public:
    ...
    void pointCloudCallback(const sensor_msgs::msg::PointCloud2::SharedPtr msg) {
        ...
        // Collect all z-values from the transformed cloud and track their frequencies
        std::unordered_map<float, int> z_value_count;

        for (const auto& point : transformed_cloud->points) {
            // Optionally round the z-values to reduce noise (e.g., to 2 decimal places)
            float z_rounded = std::round(point.z * 100.0) / 100.0;
            z_value_count[z_rounded]++;
        }

        // Find the most repeated (mode) z-value
        float mode_z = 0.0;
        int max_count = 0;
        for (const auto& [z_value, count] : z_value_count) {
            if (count > max_count) {
                max_count = count;
                mode_z = z_value;
            }
        }
        RCLCPP_INFO(this->get_logger(), "%lf", mode_z);
        
        // // Initialize variables for segment-based ground detection
        // float z_min = -1.0;
        // float z_max = -0.5;
        // float z_step = 0.05;
        // int num_segments = static_cast<int>((z_max - z_min) / z_step);  // 10 segments from -1.0 to 0.0
        // int segment_sizes[10] = {0};  // Array to store the sizes of each segment

        // pcl::PointCloud<pcl::PointXYZ>::Ptr segment_cloud(new pcl::PointCloud<pcl::PointXYZ>);

        // // Iterate through z segments and calculate the size for each segment
        // for (int i = 0; i < num_segments; ++i) {
        //     float z_start = z_min + i * z_step;
        //     float z_end = z_start + z_step;

        //     // Filter the point cloud for the current z-segment
        //     pcl::PassThrough<pcl::PointXYZ> pass_z;
        //     pass_z.setInputCloud(transformed_cloud);
        //     pass_z.setFilterFieldName("z");
        //     pass_z.setFilterLimits(z_start, z_end);
        //     pass_z.filter(*segment_cloud);

        //     // Store the size of the segment in the array
        //     segment_sizes[i] = segment_cloud->size();
        // }

        // // Find the segment with the maximum size
        // int max_segment_index = 0;
        // for (int i = 1; i < num_segments; ++i) {
        //     if (segment_sizes[i] > segment_sizes[max_segment_index]) {
        //         max_segment_index = i;
        //     }
        // }
        // // Filter the point cloud to only include points above the segment with the maximum size
        // float max_segment_z = z_min + (max_segment_index + 2) * z_step;
        // pcl::PassThrough<pcl::PointXYZ> pass_above;
        // pass_above.setInputCloud(transformed_cloud);
        // pass_above.setFilterFieldName("z");
        // pass_above.setFilterLimits(max_segment_z, std::numeric_limits<float>::max());  // Keep points above this segment
        // pass_above.filter(*transformed_cloud);  // Update the cloud to contain only points above the segment
        ...
        seg.setDistanceThreshold(0.01);  // Adjust based on precision required
        ...
        pcl_ros::transformPointCloud(*filtered_cloud, *rotated_cloud, rotate_yaw_);
        ...
        for (double segment_z = z_max; segment_z > mode_z + 0.1; segment_z -= z_step) {
            pcl::PassThrough<pcl::PointXYZ> pass_z;
...
void odomCallback(const nav_msgs::msg::Odometry::SharedPtr msg) {
    ...
    // Create a transformation that only uses the pitch and yaw value
    tf2::Transform odom_transform_pitch;
    tf2::Quaternion pitch_quat;
    pitch_quat.setRPY(0, pitch, yaw);  // Set only pitch, ignoring roll and yaw
    odom_transform_pitch.setRotation(pitch_quat);
    odom_transform_pitch.setOrigin(tf2::Vector3(0, 0, msg->pose.pose.position.z));  // Zero out the translation

    // Store the inverse of the pitch-only transformation for point cloud alignment
    odom_to_start_ = odom_transform_pitch.inverse();
    ...
    yaw_transform.setRotation(yaw_quat);
    yaw_transform.setOrigin(tf2::Vector3(0, 0, 0));  // Zero out the translation

    rotate_yaw_ = yaw_transform;
}
